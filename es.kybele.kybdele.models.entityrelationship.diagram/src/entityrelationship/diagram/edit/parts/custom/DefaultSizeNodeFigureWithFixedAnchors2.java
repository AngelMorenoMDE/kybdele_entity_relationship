...
public class DefaultSizeNodeFigureWithFixedAnchors2 extends
		DefaultSizeNodeFigure {

	private HashMap<String, FixedConnectionAnchor> anchors = new HashMap<String, FixedConnectionAnchor>();

	public DefaultSizeNodeFigureWithFixedAnchors2(Dimension defSize, HashMap<String, PrecisionPoint> anchorLocations) {
		this(defSize.width, defSize.height, anchorLocations);
	}

	public DefaultSizeNodeFigureWithFixedAnchors2(int width, int height, HashMap<String, PrecisionPoint> anchorLocations) {
		super(width, height);
	...
	public PointList getPolygonPoints() {
		...
		org.eclipse.draw2d.geometry.Rectangle anchRect = getHandleBounds();
		points.addPoint(anchRect.x + anchRect.width * 20 / 40, anchRect.y);					
		points.addPoint(anchRect.x + anchRect.width, anchRect.y	+ anchRect.height * 20 / 40);				
		points.addPoint(anchRect.x + anchRect.width * 20 / 40, anchRect.y + anchRect.height);				
		points.addPoint(anchRect.x, anchRect.y + anchRect.height * 20 / 40);				
		points.addPoint(anchRect.x + anchRect.width * 20 / 40, anchRect.y);				
